// run-aug14-1250.mjs — reads gathered/factor-gathered-aug14.json, builds HTML cards,
// prints PDFs via Chrome headless into KW-specific folders, updates generated-wos.json.
// Covers: W34 new WOs (34-215, 34-216) + ALL W35 WOs.
import fs from 'fs';
...
const DATA_FILE  = path.join(__dir, 'gathered', 'factor-gathered-aug14.json');
...
// ── HTML builder (latest version from run-aug13-1300.mjs / gather-build-aug11.mjs) ─────
const ALLERGEN_EN = {
...
for (const g of gathered) {
  ...
  // Only include subs with a new WO
...
const usedRunN = {};  // recCode → runN (to avoid double-incrementing same recipe)
...
for (const [rc, n] of Object.entries(usedRunN)) state.runCount[rc] = n;
fs.writeFileSync(STATE_FILE, JSON.stringify(state, null, 1));
log('State saved.');

log(`\n=== DONE: ${generated.length} PDFs, ${Object.keys(usedRunN).length} recipes ===`);
