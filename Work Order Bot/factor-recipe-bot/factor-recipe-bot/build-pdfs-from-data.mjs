// build-pdfs-from-data.mjs — reads gathered-data.json produced by the browser gather script,
// builds HTML cards and prints PDFs via Chrome headless, then updates generated-wos.json.
import fs from 'fs';
...
const CONFIG = {
  ...
  dataFile:  path.join(__dir, 'gathered-data.json'),
  outDir: process.env.FACTOR_OUT || 'G:\\Shared drives\\Recipe  Bible',
};

// ---- buildHtml (identical logic to bot.mjs v10) ----
...
if (!data.ok) { log('Gathered data not OK:', data.error); process.exit(1); }

const state = JSON.parse(fs.readFileSync(CONFIG.stateFile, 'utf8').replace(/^﻿/, ''));
...
for (const g of data.groups) {
  try {
    if (!g.allSubs || !g.allSubs.length) { log(`SKIP (no subs): ${g.recCode}`); continue; }
    log(`Building PDF: ${g.recName} — Run ${g.runN} (${g.allSubs.length} cards)`);
    const html = buildHtml(g.rec, g.allSubs, g.cache, `RUN ${g.runN}`);
    const safe = (g.recName + ' - Run ' + g.runN).replace(/[\\/:*?"<>|]/g, '_').slice(0, 120);
    ...
    if (outDir !== SCRATCH) {
      ...
      try { fs.unlinkSync(pdfScratch); } catch {}
    }
    try { fs.unlinkSync(htmlPath); } catch {}
    g.wos.forEach(w => state.doneWO[w] = TODAY);
    state.runCount[g.recCode] = g.runN;
    made++;
    generated.push({ file: path.basename(pdfFinal), wos: g.wos, subs: g.allSubs.map(s => `${s.name} → ${s.wo}`) });
    log(`  DONE: ${safe}`);
  } catch (e) {
...
try { fs.unlinkSync(CONFIG.dataFile); } catch {}

log('\n=== SUMMARY ===');
log(`PDFs generated: ${made}/${data.groups.length}`);
if (errors.length) { log('Errors:'); errors.forEach(e => log(' ', e)); }
log('Generated files:');
generated.forEach(g => { log(`  ${g.file}`); g.subs.forEach(s => log(`    ${s}`)); });
