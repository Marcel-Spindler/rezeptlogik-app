...
// LIMIT und Hours-Deckel: usePlatedMealTotals fragt für eine bereits
// abgeschlossene Vorwoche bis zu 576h (3 Wochen) an, siehe lookbackHoursFor in
// useCombinedPlaited.ts. Der alte Deckel (168h/500 Zeilen) war auf den
// 24h-Normalfall zugeschnitten — bei einem breiteren Fenster kappte
// "ORDER BY startTime DESC LIMIT 500" die älteren (aber noch relevanten)
// Zeilen der Vorwoche einfach weg, WÄHREND das Zeitfenster selbst schon auf
// 168h gedeckelt war: "schon plaitiert" blieb dadurch trotz breiterem Fenster
// nahe 0 (live an KW38 nach dem KW39-Wechsel beobachtet — 5.376 von 7.343
// Meals zeigten sich fälschlich noch als "platierbar").
...
exports.redzoneStatus = onGuardedRequest({ region: "europe-west3", timeoutSeconds: 60 }, async (req, res) => {
  ...
  const hours = Number.isFinite(hoursParam) ? Math.min(600, Math.max(1, hoursParam)) : 24;
...
Return JSON: {"english":"...","german":"...","status":"needs_review"}` }] },
  ...
  // `breakRepetition`: Retry-Modus für eine erkannte Wiederholungsschleife
  // (Modell schließt das JSON nie ab, füllt den Rest des Budgets stattdessen
  // mit Leerzeilen). gemini-2.5-flash lehnt frequencyPenalty/presencePenalty
  // ab ("Penalty is not enabled for..."), also bleibt nur eine deutlich
  // höhere Temperatur, um aus dem bei 0.1 fast deterministischen Loop
  // rauszukommen — mehr maxOutputTokens allein hilft nicht (der Text wächst
  // dann nur proportional mit und bleibt trotzdem abgeschnitten).
  const bodyFor = (breakRepetition) => JSON.stringify({
    ...baseRequest,
    generationConfig: {
      responseMimeType: "application/json",
      responseSchema: {
  ...
  let { text, finishReason } = await callGemini(bodyFor(false));
  if (finishReason === "MAX_TOKENS") {
    ({ text, finishReason } = await callGemini(bodyFor(true)));
    if (finishReason === "MAX_TOKENS") {
  ...
  async (req, res) => {
    ...
    try {
      ...
      const systemPrompt = [
        ...
        "Unten steht ein frischer Live-Snapshot des Plans. Zusätzlich hast du Werkzeuge, um gezielt Details nachzuladen (get_recipe_detail, get_backfill_detail, get_wo_trace, get_plating_plan, get_kitchen_plan) und Ideen risikofrei zu testen (simulate_plating_change, simulate_day_plating_change).",
        "Arbeitsweise: erst mit Werkzeugen Fakten holen bzw. eine Idee simulieren, dann antworten. Nie raten.",
        "get_kitchen_plan liefert den Kochplan: je Küchentag (So · Mo–Fr) und Schicht die zu fertigenden Sub-Meals nach Küchenbereich, allergen-aufsteigend sequenziert, mit Portionen/kg/Batches. Er wird rein aus dem Plating-Plan + Cook Schedule abgeleitet — ändern nur, indem der Plating-Plan geändert wird.",
  ...
  async (req, res) => {
    ...
    try {
      ...
      const systemPrompt = [
        ...
        "• Changeover ist rein allergen-getrieben: Allergen weg = 30min Reinigung; Allergen nur zufügen = kurzer easy Changeover; sonst 0.",
