...
export function BackfillNavBadge() {
  ...
  const count = backfills.criticalCount;
  if (count === 0 && !backfills.isStaleWeek) return null;
  return (
    <span className="relative flex h-2 w-2 shrink-0" title={count > 0 ? `${count} kritische Backfill-Frühwarnung(en)` : "Backfills zeigt veraltete Daten"}>
...
export function BackfillAlertBanner({ onOpen }: { onOpen: () => void }) {
  ...
  // Stale-Warnung hat niedrigere Prio als kritische Alerts
  const critical = backfills.alerts.filter(a => a.severity === "critical");

  if (critical.length === 0 && !backfills.isStaleWeek) return null;

  // Stale-only Banner (kein kritischer Alert, aber alte Daten)
      ...
      <div className="flex items-start justify-between gap-2">
        <button type="button" onClick={onOpen} className="text-left flex-1">
          <div className="text-xs font-bold uppercase tracking-wide text-red-800">
            {critical.length} Backfill-Frühwarnung{critical.length > 1 ? "en" : ""}
          </div>
