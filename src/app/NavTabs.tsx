...
export function NavTabs({ view, onChange }: { view: AppView; onChange: (v: AppView) => void }) {
  ...
  return (
    <nav ref={navRef} className="card p-1.5 flex flex-col gap-0.5">
      ...
      {/* Kiosk-Dashboards: eigene Surfaces, über URL-Parameter erreichbar */}
      <div className="mt-1 pt-1.5 border-t border-slate-100">
        ...
        <a
          href="?surface=staging"
          target="_blank"
          rel="noreferrer"
          className="w-full text-left px-3 py-2 text-sm rounded-lg font-medium transition-colors flex items-center gap-2 text-slate-600 hover:bg-slate-100 hover:text-slate-900"
        >
          <span className="text-base leading-none">📦</span>
          <span className="flex-1 truncate">Staging Dashboard</span>
          <span className="text-[10px] text-slate-300">↗</span>
        </a>
        <a
          href="?surface=plating"
          target="_blank"
          rel="noreferrer"
          className="w-full text-left px-3 py-2 text-sm rounded-lg font-medium transition-colors flex items-center gap-2 text-slate-600 hover:bg-slate-100 hover:text-slate-900"
        >
          <span className="text-base leading-none">🍽</span>
          <span className="flex-1 truncate">Plating Dashboard</span>
          <span className="text-[10px] text-slate-300">↗</span>
        </a>
      </div>
